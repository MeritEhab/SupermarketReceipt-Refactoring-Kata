...
class TestReceiptPrinter(unittest.TestCase):
    ...
    def test_one_line_item(self):
        ...
        self.given(function_name).expect_formatted_text_receipt()
    ...
    def test_discounts(self):
        function_name = sys._getframe().f_code.co_name
        self.receipt.add_product(self.apples, 3, 1.99, 1.99 * 3)
    ...
    def _get_receipt_comparable_result(self, file_name):
        receipts_dir = os.path.join(os.getcwd(), "tests", "receipts_output", "text_receipts")
...
class TestHtmlReceiptPrinter(unittest.TestCase):
    def setUp(self):
        self.toothbrush = Product("toothbrush", ProductUnit.EACH)
        self.apples = Product("apples", ProductUnit.KILO)
        self.receipt = Receipt()
    ...
    def test_one_line_item(self):
        function_name = sys._getframe().f_code.co_name
        self.receipt.add_product(self.toothbrush, 1, 0.99, 0.99)
        self.given(function_name).expect_formatted_html_receipt()
    ...
    def _get_receipt_comparable_result(self, file_name):
        receipts_dir = os.path.join(os.getcwd(), "tests", "receipts_output", "html_receipts")
        with open(os.path.join(receipts_dir, file_name), 'r') as f:
            return f.read()
    
    def given(self, function_name):
        self.expected_output = self._get_receipt_comparable_result(f"{self.class_name}.{function_name}.html")
        self.result = HtmlReceiptPrinter().print_receipt(self.receipt)
        return self

    def expect_formatted_html_receipt(self):
        assert_that(self.result).is_equal_to(self.expected_output)
