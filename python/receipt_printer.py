...
class HtmlReceiptPrinter:
    ...
    def _print_receipt_item(self, item):
        line = f"<tr><td>{item.product.name}</td><td>{item.total_price:.2f}</td></tr>"
        if item.quantity != 1:
            line = f"<tr><td>{item.product.name}<br>{item.price:.2f} * {self._print_quantity(item)}</td><td>{item.total_price:.2f}</td></tr>"
        return line
    ...
    def _print_discount(self, discount):
        return f"<tr><td><strong>Discount</strong></td><td>{discount.description}</td> <td>({discount.product.name}):</td> <td>{discount.discount_amount:.2f}</td></tr>"

    def _present_total(self, receipt):
        return f"<tr><td><strong>Total:</strong></td> <td><strong>{receipt.total_price():.2f}</strong></td></tr>"
